...
void * dh_init(const struct dh_group *dh, unsigned char **priv)
{
	...
	*priv = (unsigned char*)malloc(dh->prime_len);
	...
	if (os_get_random(*priv, dh->prime_len)) {
		free(*priv);
		*priv = NULL;
		if (debug >= 0)
			printf("\n Error: private value not got via random function");
		return NULL;
	}

	if (memcmp(*priv, dh->prime, dh->prime_len) > 0) {
		/* Make sure private value is smaller than prime */
		//*(*priv) = 0;
		if (debug >= 0)
			printf("\n Error: private value is larger than prime");
	}
...
void * dh_derive_shared(void *peer_public,
	 void *own_private,
	 const struct dh_group *dh, size_t *len, size_t r_dh_public_len)
{
	void *shared;
	size_t shared_len;
	...
	if (shared == NULL)
		return NULL;
	if (crypto_mod_exp(peer_public, r_dh_public_len,
			   own_private, dh->prime_len,
	...
	if (debug >= 2) {
		printf("\n");
		printf("IKEV2 Shared Key: %d: ", shared_len);
		hexdump(shared, 10);
	}
